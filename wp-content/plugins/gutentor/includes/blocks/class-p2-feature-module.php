...
if ( ! class_exists( 'Gutentor_P2_Feature_Module' ) ) {
	...
	class Gutentor_P2_Feature_Module extends Gutentor_Block_Base{
		...
		public function render_callback( $attributes, $content ) {
            ...
            $gutentor_p2_news_the_query = new WP_Query( $args );

			if ($gutentor_p2_news_the_query->have_posts()) :
				$output  .= '<'.$tag.' class="'.apply_filters('gutentor_post_module_main_wrap_class',gutentor_concat_space('section-'.$gID,'gutentor-post-module','gutentor-post-module-p2','gutentor-post-'.$post_number,'gutentor-template-'.$template), $attributes).'" id="'. esc_attr($blockID) . '" '.GutentorAnimationOptionsDataAttr($blockComponentAnimation).'>' . "\n";
